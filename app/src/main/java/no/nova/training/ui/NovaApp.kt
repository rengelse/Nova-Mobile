...
@Composable fun NovaApp(repository: NovaRepository) {
    ...
    val bottomKey = when (route) { Route.Transfer -> "transfer"; Route.More, Route.Programs -> "more"; else -> "program" }

    Scaffold(
        modifier = Modifier.fillMaxSize(),
        bottomBar = {
            if (route !is Route.Exercise) NovaBottomBar(bottomKey) { key ->
                route = when (key) { "transfer" -> Route.Transfer; "more" -> Route.More; else -> Route.Program }
            }
        }
    ) { padding ->
        when (val r = route) {
